...
Docker의 `healthcheck`와 `depends_on` 조건을 활용하여 컨테이너 실행 순서를 제어하는 실습 프로젝트입니다.

Spring Boot 애플리케이션이 MySQL이 완전히 준비되기 전에 먼저 실행될 경우 발생하는 DB 연결 실패 문제를 해결하는 것이 핵심 목표입니다.
...
| 상태 | 의미 |
|------|------|
| `starting` | 컨테이너 실행 중, healthcheck 아직 미실행 |
...
- `start_period`: 컨테이너 실행 초기 30초는 healthcheck 실패를 무시 (MySQL 초기화 시간 확보)
...
- `start_period: 60s`: Spring Boot 실행 시간을 고려하여 초기 60초는 체크 유예
...
| 상황 | 동작 |
|------|------|
| 최초 실행 시 db가 준비 안 된 경우 | app 실행 대기 → **해결됨** |
| 실행 후 db가 장애 발생한 경우 | app은 계속 실행, 요청 시 에러 → **미해결** |

`depends_on: condition: service_healthy`는 **컨테이너 시작 시점의 순서만 보장**합니다.
일단 app이 실행된 이후 DB가 다운되면, app 컨테이너는 살아있지만 DB 요청 시 에러가 발생합니다.
